#!/bin/bash
target=../lzw_rust/target/debug/lzw_rust
reference=../lzw_c/lzwCompress
for testInput in *.in; do
    ...
    echo "$name"
    $reference $(<$name.in) 1> $name.out.expect 2> $name.err.expect
    exitCode=$?
    ...
    echo "$name"
    $target $(<$name.in) 1> $name.out 2> $name.err
    exitCode=$?
    if [ $exitCode -eq 0 ]; then
